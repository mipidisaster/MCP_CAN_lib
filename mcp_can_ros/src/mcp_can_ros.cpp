 ...
 ** Descriptions:            Performs a spi transfer via ROS Service
 *********************************************************************************************************/
void MCP_CAN::spiTransfer(uint8_t *buff, uint8_t number_bytes)
{
    milibrary::BUSctrl msg;

    // Clear the data initially
    msg.request.address = spi_address;
    msg.request.read_size = 0;
    msg.request.write_data.insert(msg.request.write_data.begin(), number_bytes, 0);
...
void MCP_CAN::mcp2515_reset(void)
{
    ...
    ros::Duration(0.005).sleep(); // If the MCP2515 was in sleep mode when the reset command was issued then we need to wait a while for it to reset properly
}
...
INT8U MCP_CAN::mcp2515_requestNewMode(const INT8U newmode)
{
    ...
    while (1) {
        ...
        if ((statReg & MODE_MASK) == newmode) // We're now in the new mode
            return MCP2515_OK;
        else if ((ros::Time::now() - startTime) > ros::Duration(0.2)) // Wait no more than 200ms for the operation to complete
            return MCP2515_FAIL;
    }
}
...
INT8U MCP_CAN::mcp2515_init(const INT8U canIDMode, const INT8U canSpeed, const INT8U canClock)
{
    ...
    if (res > 0) {
#if DEBUG_MODE
    ROS_DEBUG("Entering Configuration Mode Failure...");
#endif
        return res;
    }
#if DEBUG_MODE
    ROS_DEBUG("Entering Configuration Mode Successful!");
#endif
    ...
    if (res > 0) {
#if DEBUG_MODE
    ROS_DEBUG("Setting Baudrate Failure...");
#endif
        return res;
    }
#if DEBUG_MODE
    ROS_DEBUG("Setting Baudrate Successful!");
#endif
    ...
    if (res == MCP2515_OK) {
        ...
        default:
#if DEBUG_MODE        
            ROS_DEBUG("`Setting ID Mode Failure...");
#endif           
            return MCP2515_FAIL;
            break;
        }
        ...
        if (res) {
#if DEBUG_MODE        
            ROS_DEBUG("Returning to Previous Mode Failure...");
#endif           
            return res;
        }
...
/*********************************************************************************************************
 ** Function name:           begin
 ** Descriptions:            Public function to declare controller initialization parameters (with
 **                          cfg parameters are input)
 *********************************************************************************************************/
INT8U MCP_CAN::begin(INT8U idmodeset, INT8U cfg1, INT8U cfg2, INT8U cfg3)
{
    INT8U res;

    //SPI.begin();
    res = mcp2515_init(idmodeset, cfg1, cfg2, cfg3);
    if (res == MCP2515_OK)
        return CAN_OK;
...
/*********************************************************************************************************
 ** Function name:           mcp2515_init
 ** Descriptions:            Initialize the controller (cfg externally defined)
 *********************************************************************************************************/
INT8U MCP_CAN::mcp2515_init(const INT8U canIDMode, INT8U cfg1, INT8U cfg2, INT8U cfg3)
{
...
INT8U MCP_CAN::init_Mask(INT8U num, INT8U ext, INT32U ulData)
{
...
#if DEBUG_MODE
    ROS_DEBUG("Starting to Set Mask!");
#endif
    res = mcp2515_setCANCTRL_Mode(MODE_CONFIG);
    if (res > 0) {
#if DEBUG_MODE
    ...
    if (res > 0) {
#if DEBUG_MODE
        ROS_DEBUG("Entering Previous Mode Failure...");
        ROS_DEBUG("Setting Mask Failure...");
#endif
        return res;
    }
#if DEBUG_MODE
    ROS_DEBUG("Setting Mask Successful!");
#endif
    return res;
}
...
INT8U MCP_CAN::init_Filt(INT8U num, INT8U ext, INT32U ulData)
{
    INT8U res = MCP2515_OK;
#if DEBUG_MODE
    ROS_DEBUG("Starting to Set Filter!");
#endif
    res = mcp2515_setCANCTRL_Mode(MODE_CONFIG);
    if (res > 0) {
#if DEBUG_MODE
        ROS_DEBUG("Enter Configuration Mode Failure...");
#endif
        return res;
    }
...
#if DEBUG_MODE
        ROS_DEBUG("Entering Previous Mode Failure...");
        ROS_DEBUG("Setting Filter Failure...");
#endif
        return res;
    }
#if DEBUG_MODE
    ROS_DEBUG("Setting Filter Successful!");
#endif
...
INT8U MCP_CAN::sendMsg()
{
    ...
    if ( (res1 & MCP_TXB_MLOA_M) == MCP_TXB_MLOA_M ) {
        return CAN_LOSTARBITRATION;
    }

    if ( (res1 & MCP_TXB_TXERR_M) == MCP_TXB_TXERR_M ) {
